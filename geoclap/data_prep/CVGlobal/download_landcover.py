# Download landcover data for locations provided in a csv file: "source_balanced_geotagged_sounds.csv"
...
LIMIT = 110000 #Download limit per job
...
class Downloader():
  def __init__(self,save_path,layer='land_cover', radius=512,
  ...
  def process(self, rows):
    ...
    for row in tqdm(rows):
      ...
      if not os.path.exists(filepath):
        ...
        image_url = f"{self.wms_uri}?service=WMS&version=1.1.1&request=GetMap&layers={self.layer}&styles=&width={self.width}&height={self.height}&srs=EPSG:3857&bbox={l},{b},{r},{t}&format=image/{self.im_format}"
        try:
...
def cli_main():
  ...
  parser.add_argument('--save_path', type=str,default="/storage1/fs1/jacobsn/Active/user_k.subash/data_raw/")
  parser.add_argument('--meta_path', default="/storage1/fs1/jacobsn/Active/user_k.subash/data_raw/metafiles/GeoSound/source_balanced_geotagged_sounds.csv",type=str)
  parser.add_argument('--radius', default=6400, type=int)
  ...
  parser.add_argument('--split_id', default='1', type=str,help='Split ID for blocks of $LIMIT sound IDs')
  ...
  downloader = Downloader(args.save_path,layer='land_cover', radius=args.radius,
