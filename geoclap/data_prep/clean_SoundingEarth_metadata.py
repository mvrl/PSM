...
def get_detailed_metadata(data_path):
    meta_df = pd.read_csv(os.path.join(cfg.data_path,'/aporee/metadata.csv'))
    print("Original data",len(meta_df))
    corrupt_ids = list(pd.read_csv(os.path.join(cfg.data_path,"/aporee/corrupt_ids_final.csv"))['long_key']) #Ignore some IDs whose mp3 is found to be corrupt
    ...
    meta_df.to_csv(os.path.join(cfg.data_path,"/aporee/final_metadata.csv"))
    ...
    metadata.to_csv(os.path.join(data_path,'/aporee/final_metadata_with_captions.csv'))
