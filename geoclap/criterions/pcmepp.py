...
class ClosedFormSampledDistanceLoss(nn.Module):
    ...
    def forward(self, img_emb, cap_emb, matched=None):
        ...
        loss_dict = loss_fn(img_emb, cap_emb, matched=matched)
        # NOTE: Efficient implementation for
        # when i2t loss and t2i loss are the same (https://github.com/naver-ai/pcme/issues/3)
        loss = 2 * loss_dict['loss'] + self.vib_beta * vib_loss

        loss_dict_out = {}
        loss_dict_out['loss/mu_pdist'] = loss_dict['loss/mu_pdist']
        loss_dict_out['loss/sigma_pdist'] = loss_dict['loss/sigma_pdist']
        loss_dict_out['loss/loss'] = loss
        loss_dict_out['criterion/shift']= self.shift
        loss_dict_out['criterion/negative_scale']= self.negative_scale
        loss_dict_out['loss/n_pseudo_gts'] = loss_dict['n_pseudo_gts']

        if self.vib_beta != 0:
            loss_dict_out['loss/vib_loss'] = vib_loss
        ...
        if self.smoothness_alpha:
            ...
            loss_dict_out['loss/loss'] = loss
            loss_dict_out['loss/n_pseudo_gts'] = smooth_i2t_loss['n_pseudo_gts'] + smooth_t2i_loss['n_pseudo_gts']
...
if __name__ == '__main__':
    ...
    loss = ClosedFormSampledDistanceLoss(smoothness_alpha=0.1, vib_beta=0.1)
    modality1_embed_dict ={}
    modality1_embed_dict['mean'] = l2normalize(torch.randn(5,1024))
    modality1_embed_dict['std'] = torch.randn(5,1024)
    
    modality2_embed_dict ={}
    modality2_embed_dict['mean'] = l2normalize(torch.randn(5,1024))
    modality2_embed_dict['std'] = torch.randn(5,1024)
    l, l_dict = loss(modality1_embed_dict, modality2_embed_dict)
